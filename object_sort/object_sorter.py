...
class Survive(Behaviour):
    ...
    def should_run(self):
        return facing_plant(small_image_sensor.get_image())
    def run(self):
...
def facing_red_plant(raw_img):
    ...
    condition1 = np.logical_and(
        np.logical_and(raw_img[:middle_idx, :, 0] >= 115, raw_img[:middle_idx, :, 0] <= 126),
        np.logical_and(raw_img[:middle_idx, :, 1] >= 0, raw_img[:middle_idx, :, 1] <= 8),
        np.logical_and(raw_img[:middle_idx, :, 2] >= 0, raw_img[:middle_idx, :, 2] <= 8)
    )
...
def having_block(raw_img):
    return having_brown_block(raw_img) or having_black_block(raw_img) or having_green_block(raw_img)
...
## Initiate scheduler
scheduler = Scheduler([survive,
                       avoid_walls,
                       battery,
                       avoid_walls_with_block,
                       find_block,
                       grab_block,
                       compress_block,
                       find_red_plant,
...
while True:
    ...
    #print(facing_plant(small_image_sensor.get_image()))
    
    #if seeing_block(small_image_sensor.get_image()):
        #show_image(small_image_sensor.get_image())
    #show_image(top_image_sensor.get_image())
    #print(close_to_wall(top_image_sensor.get_image()))
    #print(seeing_red_plant(top_image_sensor.get_image()))
    #print(seeing_brown_block(small_image_sensor.get_image()))
    #print(seeing_block(small_image_sensor.get_image()))
    #print(facing_red_plant(small_image_sensor.get_image()))
    #print(facing_green_plant(small_image_sensor.get_image()))
    #print(f"Facing plant: {facing_plant(small_image_sensor.get_image())}")